  ...
  return defineTool({
    ...
    parameters: {
      ...
      outputDir: {
        type: 'string',
        description:
          'Directory to write the .xlsx into. Falls back to the plugin defaultOutputDir or the current working directory.',
      },
      fileName: {
        type: 'string',
      ...
      sheetPerTable: {
        type: 'boolean',
        description:
          'If true, each table gets its own worksheet; otherwise all tables are stacked into one "Tables" sheet.',
      },
    },
    output: {
      schema: {
    ...
    async execute(args, exec) {
      const result = exportMarkdownTablesToExcel({
        ...
        sheetPerTable:
          args.sheetPerTable !== undefined ? args.sheetPerTable : config.sheetPerTable,
      })
