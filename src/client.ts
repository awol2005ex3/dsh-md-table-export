...
interface SheetJsLike {
  utils: {
    book_new(): Workbook
    book_append_sheet(wb: Workbook, ws: unknown, sheetName?: string): void
    table_to_book(table: HTMLTableElement, opts?: { raw?: boolean }): Workbook
    table_to_sheet(table: HTMLTableElement, opts?: { raw?: boolean }): unknown
  }
...
function alertError(err: unknown, prefix: string): void {
...
let sheetJsPromise: Promise<SheetJsLike> | undefined

function loadSheetJs(): Promise<SheetJsLike> {
  if (win.XLSX !== undefined) return Promise.resolve(win.XLSX)
  if (sheetJsPromise !== undefined) return sheetJsPromise
  sheetJsPromise = new Promise<SheetJsLike>((resolve, reject) => {
    const script = document.createElement('script')
    script.src = SHEETJS_CDN
    script.onload = () => {
      if (win.XLSX !== undefined) resolve(win.XLSX)
      else reject(new Error('SheetJS 已加载但未暴露 XLSX 全局对象'))
    }
...
async function exportTable(table: HTMLTableElement): Promise<void> {
  try {
...
function processTable(table: HTMLTableElement): void {
  if (table.dataset.dshProcessed === '1') return
...
function ensureToolbar(): void {
  if (document.getElementById(ALL_BTN_ID) !== null) return
